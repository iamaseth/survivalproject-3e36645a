...
function CreatorLine({c}:{c:CreatorRow}) { const [details,setDetails]=useState(false); const followers=c.followersSignal||c.reachSignal||"—"; return <div className="border-b border-border last:border-0"><div className="grid items-center gap-2 px-4 py-3 md:grid-cols-[minmax(190px,1.5fr)_minmax(260px,1.2fr)_1fr_34px]"><div><div className="font-medium">{c.name}</div><div className="text-xs text-muted-foreground">{followers}</div></div><div className="min-w-0">{c.youtube?<><a {...externalLinkProps(c.youtube)} className="inline-flex items-center gap-1 rounded-md border border-input px-2 py-1 text-xs"><Youtube className="h-3.5 w-3.5"/>YouTube</a><div className="mt-1 select-all break-all text-[11px] text-muted-foreground" title="Select and copy this URL into your browser">{c.youtube}</div></>:<span className="text-xs text-muted-foreground">No YouTube URL</span>}</div><div className="flex flex-wrap gap-2 text-xs">{c.email?<a href={`mailto:${c.email}`} className="inline-flex items-center gap-1"><Mail className="h-3.5 w-3.5"/>{c.email}</a>:c.contactRoute?.startsWith("http")?<a {...externalLinkProps(c.contactRoute)} className="inline-flex items-center gap-1 underline">Contact<ExternalLink className="h-3 w-3"/></a>:<span className="text-muted-foreground">No direct email yet</span>}</div><button onClick={()=>setDetails(v=>!v)} className="p-1">{details?<ChevronDown className="h-4 w-4"/>:<ChevronRight className="h-4 w-4"/>}</button></div>{details&&<div className="border-t border-border bg-secondary/20 px-4 py-4 grid gap-2 md:grid-cols-2 text-sm"><Detail label="YouTube" value={c.youtube}/><Detail label="Instagram" value={c.instagram}/><Detail label="Facebook" value={c.facebook}/><Detail label="TikTok" value={c.tiktok}/><Detail label="Email" value={c.email}/><Detail label="Contact" value={c.contactRoute}/><Detail label="Other" value={c.otherPlatform}/><Detail label="Verification" value={c.fullVerification}/><Link to="/creators/$id" params={{id:c.id}} className="text-xs underline">Full creator details</Link></div>}</div>; }
function Detail({label,value}:{label:string;value:string|null}) { if(!value)return null; const link=value.startsWith("http"); return <div className="flex gap-2"><span className="w-24 shrink-0 text-xs text-muted-foreground">{label}</span>{link?<><a {...externalLinkProps(value)} className="break-all underline">{value}</a></>:<span className="break-words">{value}</span>}</div>; }
