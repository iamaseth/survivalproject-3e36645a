...
const CleanupInvalidEmailsSchema = z.object({
  action: z.literal("cleanup_known_invalid_emails"),
});

const KNOWN_INVALID_ENRICHMENT_EMAILS = new Set([
  "user@domain.com",
  "flags@2x.png",
]);
...
function getClassification(externalLinks: unknown): CandidateClassification | null {
  ...
  return ["creator", "brand_company", "competitor", "organization", "needs_review"].includes(value) ? value as CandidateClassification : null;
}
function withClassification(externalLinks: unknown, classification: CandidateClassification) {
  ...
  return [...existing.filter((item) => item.kind !== "research_classification"), { kind: "research_classification", url: null, source: classification }];
}

function safeAutoClassification(row: { channel_title?: string | null; topic_keyword?: string | null; video_count?: number | null; subscriber_count?: number | null; last_upload_at?: string | null; notes?: string | null; }): CandidateClassification | null {
  ...
  const competitor = ["legacy food storage", "legacyfoodstorage", "readywise", "4patriots", "my patriot supply", "augason farms", "mountain house", "nutristore", "valley food storage"];
  if (competitor.some((term) => haystack.includes(term))) return "competitor";
  const organizationPatterns = [/\bfema\b/, /\bred cross\b/, /\bemergency management agency\b/, /\boffice of emergency management\b/, /\bdepartment of emergency management\b/, /\bcommunity emergency response team\b/, /\bnational weather service\b/, /\bfire department\b/, /\bpolice department\b/, /\bsheriff'?s office\b/, /\bcounty emergency\b/, /\bcity of [a-z]/, /\bstate of [a-z]/, /\buniversity extension\b/, /\bcivil defense\b/];
  if (organizationPatterns.some((rx) => rx.test(title))) return "organization";
  const companyPatterns = [/\b(inc\.?|llc|ltd\.?|corp\.?|corporation)\b/, /\bofficial channel\b/, /\bequipment company\b/, /\boutdoor products\b/, /\bpreparedness store\b/, /\bsurvival store\b/, /\bgear shop\b/, /\bsupply company\b/, /\bmanufacturing\b/, /\btechnologies\b/];
  ...
  const creatorStylePatterns = [/\bwith [a-z]/, /\bmy (?:homestead|adventures?|journey|life|prep)/, /\b[a-z]+['’]s (?:edc|homestead|adventures?|survival|prep)/, /\b(?:homesteading|off[ -]?grid living|van life|rv life) with\b/, /\b(?:prepper|survival|bushcraft|outdoors?) podcast\b/, /\b(?:gear|camping|hiking|backpacking|edc) reviews?\b/, /\bfamily (?:homestead|preparedness|adventures?)\b/];
...
async function applyEnrichmentRows(rows: z.infer<typeof ResultSchema>[]) {
  ...
  for (const item of rows) {
    const { data: current, error: readErr } = await supabaseAdmin.from("youtube_candidates").select("id,business_email,description_email,email_source,external_links").eq("id", item.id).maybeSingle();
    ...
    if (proposedEmail) {
      ...
      else if (!existingEmail && sourceUrl) { newBusinessEmail = proposedEmail; newEmailSource = sourceUrl; emailAdded += 1; }
    }
    ...
    const { error: updateErr } = await supabaseAdmin.from("youtube_candidates").update({ business_email: newBusinessEmail, email_status: newBusinessEmail || (current as { description_email?: string | null }).description_email ? "found" : item.status === "error" ? "error" : "none", email_source: newEmailSource, external_links: merged.merged, enrichment_status: foundAnything ? "found" : item.status, enrichment_checked_at: new Date().toISOString(), enrichment_error: item.status === "error" ? item.error ?? "Unknown enrichment error" : null } as never).eq("id", item.id);
...
async function cleanupKnownInvalidEmails() {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("youtube_candidates").select("id,business_email,description_email,email_source,enrichment_status").eq("status", "pending");
  if (error) throw new Error(error.message);
  let examined = 0, cleaned = 0;
  const cleanedIds: string[] = [];
  for (const row of data ?? []) {
    examined += 1;
    const business = String((row as { business_email?: string | null }).business_email || "").trim().toLowerCase();
    const description = String((row as { description_email?: string | null }).description_email || "").trim().toLowerCase();
    const badBusiness = KNOWN_INVALID_ENRICHMENT_EMAILS.has(business);
    const badDescription = KNOWN_INVALID_ENRICHMENT_EMAILS.has(description);
    if (!badBusiness && !badDescription) continue;
    const { error: updateErr } = await supabaseAdmin.from("youtube_candidates").update({
      business_email: badBusiness ? null : (row as { business_email?: string | null }).business_email,
      description_email: badDescription ? null : (row as { description_email?: string | null }).description_email,
      email_source: badBusiness || badDescription ? null : (row as { email_source?: string | null }).email_source,
      email_status: "none",
    ...
    if (updateErr) throw new Error(updateErr.message);
    cleaned += 1;
    cleanedIds.push((row as { id: string }).id);
  }
  return { ok: true, examined, cleaned, cleaned_ids: cleanedIds };
}
...
async function runSafeClassification(limit: number) {
  ...
  for (const raw of data ?? []) {
    ...
    const { error: updateErr } = await supabaseAdmin.from("youtube_candidates").update({ external_links: withClassification(row.external_links, classification) } as never).eq("id", row.id);
    if (updateErr) throw new Error(updateErr.message);
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const { data, error } = await supabaseAdmin.from("youtube_candidates").select("id,channel_id,channel_url,channel_title,subscriber_count,country,topic_keyword,last_upload_at,enrichment_status").eq("status", "pending").is("business_email", null).is("description_email", null).in("enrichment_status", ["not_started", "error"]).order("subscriber_count", { ascending: false, nullsFirst: false }).order("last_upload_at", { ascending: false, nullsFirst: false }).limit(limit);
      ...
      POST: async ({ request }) => {
        ...
        const cleanupRequest = CleanupInvalidEmailsSchema.safeParse(raw);
        if (cleanupRequest.success) {
          try { return json(await cleanupKnownInvalidEmails()); }
          catch (error) { return json({ error: error instanceof Error ? error.message : "Cleanup failed" }, 500); }
        }

        const classificationRequest = SafeClassificationSchema.safeParse(raw);
        if (classificationRequest.success) {
          try { return json(await runSafeClassification(classificationRequest.data.limit)); }
          catch (error) { return json({ error: error instanceof Error ? error.message : "Classification failed" }, 500); }
        }
        const batch = EnrichmentBatchSchema.safeParse(raw);
        if (batch.success) {
          try { return json({ ok: true, ...(await applyEnrichmentRows(batch.data.rows)) }); }
          catch (error) { return json({ error: error instanceof Error ? error.message : "Enrichment failed" }, 500); }
        }
        const single = ResultSchema.safeParse(raw);
        if (single.success) {
          try { return json({ ok: true, ...(await applyEnrichmentRows([single.data])) }); }
          catch (error) { return json({ error: error instanceof Error ? error.message : "Enrichment failed" }, 500); }
        }
