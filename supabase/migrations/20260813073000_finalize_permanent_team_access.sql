-- Final permanent team-access repair.
-- New migration intentionally follows the earlier roster migration so databases
-- that already recorded the earlier filename still receive these fixes.
...
DECLARE
  ...
  _display text;
BEGIN
  IF _uid IS NULL OR _email = '' THEN RETURN NULL; END IF;

  SELECT role, display_name INTO _role, _display
...
BEGIN
  ...
  IF _assigned IS NULL THEN RETURN NEW; END IF;
